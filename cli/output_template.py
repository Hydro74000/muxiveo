...
def build_output_context(
    ...
    *,
    ...
    return {
        ...
        "release_group": sanitize_token((variables or {}).get("release_group", extract_release_group(source_path.stem))),
        "hybrid_tag": "Hybrid" if len({item[0] for item in (track_order or [])}) > 1 else "",
        "channels_str": next((_audio_channels_label(t) for t in (tracks or []) if t.enabled and t.track_type == "audio"), ""),
