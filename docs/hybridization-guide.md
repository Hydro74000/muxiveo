...
- `--cadence-method {auto,atempo,asetrate}` : Choisit la méthode de conversion audio lors d'un décalage de cadence :
  - `auto` (défaut) : analyse acoustique automatique de la hauteur tonale (F0 et densité spectrale de Welch) pour discriminer automatiquement entre PAL speedup historique non compensé (`asetrate`) et diffusion broadcast préservant la tonalité (`atempo`).
  - `atempo` : préserve la hauteur et le timbre d'origine des voix et de la musique (*time-stretch*).
